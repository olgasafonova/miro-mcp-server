...
func (c *Client) UploadDocument(ctx context.Context, args UploadDocumentArgs) (UploadDocumentResult, error) {
	...
	dataBytes, err := json.Marshal(dataJSON)
	if err != nil {
		return UploadDocumentResult{}, fmt.Errorf("failed to marshal data: %w", err)
	}

	dataPart, err := writer.CreateFormField("data")
	if err != nil {
		return UploadDocumentResult{}, fmt.Errorf("failed to create data field: %w", err)
	}
	if _, err := dataPart.Write(dataBytes); err != nil {
		return UploadDocumentResult{}, fmt.Errorf("failed to write data: %w", err)
	}

	// Add file resource part
	resourcePart, err := writer.CreateFormFile("resource", filepath.Base(args.FilePath))
	if err != nil {
		return UploadDocumentResult{}, fmt.Errorf("failed to create resource field: %w", err)
	}
	if _, err := io.Copy(resourcePart, file); err != nil {
		return UploadDocumentResult{}, fmt.Errorf("failed to write file data: %w", err)
	}

	if err := writer.Close(); err != nil {
		return UploadDocumentResult{}, fmt.Errorf("failed to close multipart writer: %w", err)
	}
